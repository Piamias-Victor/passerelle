...
/usr/bin/env npx ts-node src/cmd.ts >> logs/run-cmd.log 2>&1

# Vérifier l'état de sortie et loguer si nécessaire
if [ $? -eq 0 ]; then
  echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO]: run-cmd.sh exécuté avec succès." >> logs/run-cmd.log
else
  echo "$(date '+%Y-%m-%d %H:%M:%S') [ERROR]: run-cmd.sh a échoué." >> logs/run-cmd.log
fi
